...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles ro WHERE ro.nom = 'Harley Quinn'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a " +
				"JOIN a.roles r " +
				"JOIN r.film f " +
				"JOIN f.realisateurs rea " +
				"WHERE rea.identite = 'Ridley Scott' " +
				"AND f.annee BETWEEN 2010 AND 2020", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireRealisateursParActeur() {
		TypedQuery<Realisateur> query = em.createQuery("SELECT r FROM Realisateur r " +
				"JOIN r.films f " +
				"JOIN f.roles ro " +
				"JOIN ro.acteur a " +
